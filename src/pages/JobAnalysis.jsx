...
export default function JobAnalysis() {
    ...
    async function autofillFromUrl() {
        ...
        setError("");
        try {
            // Using our backend job analyze which Simon can use to extract data too
            const res = await jobAPI.analyze(`URL: ${jobUrl}`);
    ...
    const analyzeJobPosting = async () => {
        ...
        try {
            // Using Simon Agent on the backend for analysis
            const response = await jobAPI.analyze(jobDescription, masterResumeRecord);

            // Save to database, including the full LLM analysis result for future reference
            const savedApplication = await jobApplicationAPI.create({
                job_title: jobTitle,
            ...
            await jobApplicationAPI.update(savedApplication.id, {
            ...
            const refreshed = await jobApplicationAPI.getById(savedApplication.id);
