...
export default function QAAssistant() {
    ...
    const loadJobApplications = async () => {
        try {
            const applications = await jobApplicationAPI.getAll({ sort: "-created_date", limit: 20 });
    ...
    const loadMasterResume = async () => {
        try {
            const masters = await resumeAPI.getAll({ is_master_resume: true, limit: 1 });
            if (masters && masters[0]) {
                setMasterResume(masters[0]);
            }
        } catch (error) {
    ...
    const generateAnswers = async () => {
        ...
        try {
            ...
            // Parse resume data
            const resumeData = masterResume.optimized_content 
                ? JSON.parse(masterResume.optimized_content)
                : JSON.parse(masterResume.parsed_content);

            // Using Backend API (Kyle Agent)
            const response = await interviewAPI.answerQuestions(
                resumeData,
            ...
            await jobApplicationAPI.update(selectedJobId, {
