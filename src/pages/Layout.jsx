...
const PRAGUE_DAY_CIRCLE = "/vite.svg";
const PRAGUE_DAY_FULL = "/vite.svg";
...
function AppShell({ children, currentPageName }) {
    ...
    const isPublicPage = ["", "Home", "Auth", "Pricing", "NotFound"].includes(currentPageName) || 
                         ["/", "/Home", "/auth", "/Auth", "/Pricing", "/pricing", "/NotFound"].includes(location.pathname);

    console.log("Layout Debug:", { currentPageName, pathname: location.pathname, isPublicPage, hasUser: !!user });
    ...
    if (isPublicPage && !user) {
        return <div className="app-main min-h-screen">{children}</div>;
    }
                    ...
                    <SidebarFooter className="border-t border-slate-200/60 p-3 md:p-4">
                        <div className="flex items-center gap-2 md:gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="font-medium text-slate-800 text-sm truncate">{user?.name || 'Prague Day'}</p>
                                <p className="text-xs text-slate-500 truncate">{user?.subscription_tier?.toUpperCase() || 'AI Career Assistant'}</p>
                            </div>
                        </div>
                    </SidebarFooter>
                </Sidebar>
                            ...
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <Button variant="ghost" className="flex items-center gap-2 hover:bg-slate-100">
                                        ...
                                        <span className="text-sm font-medium text-slate-700">{user?.name || 'My Account'}</span>
                                    </Button>
                                </DropdownMenuTrigger>
